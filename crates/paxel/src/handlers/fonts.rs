...
pub async fn get_fonts() -> impl IntoResponse {
    let output = match Command::new("fc-list").output() {
        ...
        Err(e) => return (
            ...
            Json(ErrorResponse { message: format!("Failed to execute fc-list: {}", e), output: None })
        ).into_response(),
    };
    ...
    let debug_fonts = std::env::var("PAXEL_DEBUG_FONTS").map(|v| v == "1").unwrap_or(false);
    ...
    for line in stdout.lines() {
        ...
        if parts.len() >= 2 {
            ...
            fonts.push(FontInfo {
                ...
                styles,
                file_name,
                raw: if debug_fonts { Some(line.to_string()) } else { None },
            });
        }
    }
